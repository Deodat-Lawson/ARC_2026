...
const povCanvas = document.querySelector("#povCanvas");
const droneSwitcher = document.querySelector("#droneSwitcher");
const povTitleEl = document.querySelector("#povTitle h2") || document.querySelector(".map-panel h2");
...
fetch("scenario_canvas_lite.json")
...
function init3D(scenario) {
  ...
  try {
    ...
    console.warn("WebGL unavailable — FPV viewport disabled.", err);
    povCanvas.replaceWith(Object.assign(document.createElement("div"), {
      style: "padding: 20px; color: #ff8c00; font-size: 11px; text-align: center;",
      textContent: "WebGL unavailable — FPV viewport disabled."
    }));
    return;
  }
  ...
  ro.observe(povCanvas.parentElement);
  resize3D();
