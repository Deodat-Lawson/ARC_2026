...
// ── ARC Bridge Configuration ──────────────────────────────────
const ARC_BRIDGE_URL = "http://localhost:8000";
let arcBridgeAvailable = false;
...
fetch("scenario_001.json")
  ...
  .then(async (scenario) => {
    ...
    // Try to connect to ARC Bridge
    try {
      const initRes = await fetch(`${ARC_BRIDGE_URL}/api/init`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(scenario),
      });
      if (initRes.ok) {
        arcBridgeAvailable = true;
        const info = await initRes.json();
        console.log("✅ ARC Bridge connected:", info);
      }
    } catch (e) {
      console.warn("⚠️ ARC Bridge not available, using local JS logic.", e.message);
      arcBridgeAvailable = false;
    }
    reset();
  });
...
function reset() {
  ...
  stopAuto();
  render();
  // Re-init bridge on reset
  if (arcBridgeAvailable) {
    fetch(`${ARC_BRIDGE_URL}/api/init`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
async function step() {
  ...
  // Try ARC Bridge for decision, fallback to local JS
  if (arcBridgeAvailable) {
    try {
      const res = await fetch(`${ARC_BRIDGE_URL}/api/step`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (res.ok) {
        plan = await res.json();
        // Mark briefing as ARC-powered
        if (plan.commander_briefing && !plan.commander_briefing.startsWith("[ARC")) {
          plan.commander_briefing = "[ARC] " + plan.commander_briefing;
        }
      } else {
        plan = generatePlan();
      }
  ...
  // Generate display plan (either from ARC or local)
  if (!arcBridgeAvailable) {
    plan = generatePlan();
  }
  render();
