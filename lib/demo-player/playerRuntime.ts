// @ts-nocheck
/* ==========================================================================
   A.R.C. Demo Player — MapLibre + tactical canvas (bundled for Next.js)
   ========================================================================== */

import maplibregl from "maplibre-gl";
import { Protocol, PMTiles } from "pmtiles";

export function initDemoPlayer(timelineUrl = "/demo-player/timeline.json") {
...
function getPmtilesArchiveUrl() {
  return `${window.location.origin}/api/pmtiles-proxy`;
}
...
function makeFallbackBasemapStyle() {
  return {
    ...
    sources: {
      protomaps: {
        type: 'vector',
        url: `pmtiles://${getPmtilesArchiveUrl()}`,
...
function initBasemap() {
  ...
  try {
    if (!pmtilesProtocolInstalled) {
      ...
      const archive = new PMTiles(getPmtilesArchiveUrl());
...
function teardownBasemapAndProtocol() {
  if (baseMap) {
    try {
      baseMap.remove();
    } catch (e) {
      console.warn("[basemap] remove failed:", e);
    }
    baseMap = null;
    baseMapReady = false;
  }
  if (pmtilesProtocolInstalled) {
    ...
    pmtilesProtocolInstalled = false;
  }
}
...
fetch(`${timelineUrl}?ts=${Date.now()}`, { cache: "no-store" })
  .then(r => {
  ...
  .then(data => {
    ...
    activePlayer = new ARCPlayer(data);
    window._player = activePlayer;
  })
  .catch(err => {
    const el = document.getElementById("thinking-text");
    if (el) {
      el.textContent =
        ...
        "请从仓库根目录运行:\n  python -m arc_core.simulation.timeline_generator \\\n      --steps 200 --output public/demo-player/timeline.json\n\n" +
        "或输出到旧路径后复制到 public/demo-player/timeline.json，然后刷新本页。\n\n" +
        err.message;
    }
  });
  ...
  return () => {
    ...
    window._player = null;
    teardownBasemapAndProtocol();
    roadSegments = [];
    roadNetworkReady = false;
    roadCapturePosted = false;
    mapCtx = null;
    chartCtx = null;
    prevFrame = null;
