...
function buildAgentSelector(agents) {
  if (!ui3d.agentSelectorHost) return;
...
function cycleSelection(direction) {
  const entry = povs[0];
  if (!entry || ui3d.agentCardEls.size === 0) return;
...
function currentTargetFor(agent, state, plan) {
...
function currentTargetIdFor(agent, plan) {
  if (!plan || !plan.mission_plan) return null;
  const action = plan.mission_plan.find((m) => m.agent === agent.id);
